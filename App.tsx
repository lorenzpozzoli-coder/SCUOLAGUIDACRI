  ...
  return (
    <div className="min-h-screen pb-12 bg-slate-50">
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 min-h-14 sm:h-16 flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-shrink-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                <h1 className="text-sm sm:text-lg font-bold text-gray-900 leading-tight truncate">PROGRAMMA LEZIONI</h1>
              ...
              <p className="text-[9px] sm:text-[10px] text-gray-500 font-bold uppercase tracking-wide hidden sm:block">Scuola Guida C.R.I.</p>
            </div>
          </div>

          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            {loading && <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>}
            <div className="flex items-center bg-gray-100 p-0.5 sm:p-1 rounded-xl">
              <button 
                onClick={() => setRole(UserRole.STUDENT)}
                className={`px-2 sm:px-3 py-1.5 text-[10px] sm:text-xs font-bold rounded-lg transition-all touch-manipulation ${role === UserRole.STUDENT ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                ALLIEVO
              </button>
              <button 
                onClick={() => role !== UserRole.INSTRUCTOR && setShowPasswordModal(true)}
                className={`px-2 sm:px-3 py-1.5 text-[10px] sm:text-xs font-bold rounded-lg transition-all touch-manipulation ${role === UserRole.INSTRUCTOR ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                ISTRUTTORE
              </button>
            </div>
            {role === UserRole.INSTRUCTOR && (
              <button 
                onClick={() => setRole(UserRole.STUDENT)} 
                className="text-[10px] sm:text-xs font-bold text-red-600 hover:bg-red-50 px-1.5 sm:px-2 py-1 rounded touch-manipulation"
              >
                Esci
              </button>
            )}
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 mt-0 sm:mt-6">
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h3 className="text-base sm:text-lg font-bold text-gray-800 flex items-center gap-2">
                ...
                {role === UserRole.STUDENT && <span className="text-[9px] sm:text-[10px] bg-emerald-100 text-emerald-700 px-1.5 sm:px-2 py-0.5 rounded-full">Sola Lettura</span>}
              </h3>
              <div className="flex items-center gap-2">
                 <button onClick={loadData} className="p-2 sm:p-1.5 text-gray-400 hover:text-blue-600 transition-colors touch-manipulation" aria-label="Aggiorna">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 sm:p-8 my-auto max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-3 sm:gap-4">
              ...
              <button type="button" onClick={handleInstructorAccess} className="flex-1 py-3.5 sm:py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-colors touch-manipulation min-h-[48px]">Accedi</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 sm:p-8 my-auto max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl sm:text-2xl font-bold mb-2 text-red-600">Conferma Eliminazione</h3>
            ...
            <input
              ...
              className="w-full border border-red-100 rounded-xl px-4 py-3.5 sm:py-3 mb-4 outline-none focus:ring-2 focus:ring-red-500 bg-red-50/30 transition-all text-base min-h-[48px] touch-manipulation"
              placeholder="Password Sicurezza"
              autoFocus
            />
            {deleteError && <p className="text-red-500 text-xs mb-4 font-bold">{deleteError}</p>}
            <div className="flex gap-3 sm:gap-4">
              <button type="button" onClick={() => setShowDeleteModal(false)} className="flex-1 py-3.5 sm:py-3 border border-gray-200 rounded-xl font-bold text-gray-600 hover:bg-gray-50 touch-manipulation min-h-[48px]">Annulla</button>
              <button type="button" onClick={handleConfirmDelete} className="flex-1 py-3.5 sm:py-3 bg-red-600 text-white font-bold rounded-xl shadow-lg hover:bg-red-700 transition-colors touch-manipulation min-h-[48px]">Elimina</button>
            </div>
          </div>
        </div>
      )}
