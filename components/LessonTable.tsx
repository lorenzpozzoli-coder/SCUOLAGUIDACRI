                ...
                {bookingId === lesson.id ? (
                  <div className="flex items-center gap-2 animate-in slide-in-from-left-2 duration-200">
                    <input
                      ...
                      className="border-2 border-blue-500 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-200 outline-none shadow-sm w-48 bg-blue-50 font-semibold"
                      autoFocus
                    />
                  </div>
                ) : (
                  <div 
                    ...
                    className={`
                      inline-flex items-center justify-center min-w-[100px] px-3 py-1.5 rounded-lg transition-all duration-200 border
                      ${lesson.student 
                        ? "text-gray-900 font-bold border-transparent bg-gray-50" 
                        : "text-gray-400 italic border-transparent"} 
                      ${canBook && !lesson.student 
                        ? "cursor-pointer bg-blue-600 text-white border-blue-600 hover:bg-blue-700 hover:scale-105 hover:shadow-lg font-bold not-italic shadow-sm" 
                        : ""}
                    `}
                    title={canBook && !lesson.student ? "Clicca qui per prenotarti!" : ""}
                  >
              ...
              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div className="flex items-center justify-end gap-2">
                  {canBook && !lesson.student && bookingId === lesson.id && (
                    <>
                      <button 
                        onClick={() => submitBooking(lesson.id)}
                        className="text-white bg-green-600 hover:bg-green-700 px-4 py-1.5 rounded-lg text-xs font-bold shadow-md transition-all active:scale-95"
                      >
                        CONFERMA
                      </button>
                      <button 
                        onClick={() => setBookingId(null)}
                        className="text-gray-600 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg text-xs font-bold transition-all"
                      >
                        X
                      </button>
                    </>
